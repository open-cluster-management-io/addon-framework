...
var _ = ginkgo.Describe("Agent deploy", func() {
	...
	ginkgo.It("Should deploy and delete agent on hosting cluster successfully", func() {
		...
		testHostedAddonImpl.prober = &agentv1alpha1.HealthProber{
	...
	ginkgo.It("Should deploy agent on hosting cluster and get available with prober func", func() {
		...
		gomega.Expect(err).ToNot(gomega.HaveOccurred())
		testHostedAddonImpl.manifests[managedClusterName] = []runtime.Object{obj}
		testHostedAddonImpl.prober = newV1alpha1DeploymentProber(types.NamespacedName{Name: "nginx-deployment", Namespace: "default"})
	...
	ginkgo.It("Should deploy and delete agent using customized hosted info func", func() {
		...
		testHostedAddonImpl.hostInfoFn = getHostedModeInfoV1alpha1
	})
