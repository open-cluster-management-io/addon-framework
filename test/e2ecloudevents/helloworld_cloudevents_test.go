...
const (
	addonName                             = "helloworldcloudevents"
...
var _ = ginkgo.Describe("install/uninstall helloworldcloudevents addons", func() {
	...
	ginkgo.It("addon should be worked", func() {
		...
		gomega.Eventually(func() error {
			agentDeploy, err := hubKubeClient.AppsV1().Deployments(addonInstallNamespace).Get(context.Background(), "helloworldcloudevents-agent", metav1.GetOptions{})
			if err != nil {
				return err
			}
	...
	ginkgo.It("addon registraion agent install namespace should work", func() {
		...
		gomega.Eventually(func() error {
			_, err := hubKubeClient.CoreV1().Secrets(agentInstallNamespaceConfig).Get(
				context.Background(), "helloworldcloudevents-hub-kubeconfig", metav1.GetOptions{})
			return err
		}, eventuallyTimeout, eventuallyInterval).ShouldNot(gomega.HaveOccurred())
