package helloworld_cloudevents
...
func TestManifestAddonAgent(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			managedClusterAddOn: addontesting.NewAddon("helloworldcloudevents", "cluster1"),
			configs:             []runtime.Object{},
			verifyDeployment: func(t *testing.T, objs []runtime.Object) {
				deployment := findHelloWorldCloudEventsDeployment(objs)
				if deployment == nil {
					t.Fatalf("expected deployment, but failed")
				}

				if deployment.Name != "helloworldcloudevents-agent" {
...
func findHelloWorldCloudEventsDeployment(objs []runtime.Object) *appsv1.Deployment {
