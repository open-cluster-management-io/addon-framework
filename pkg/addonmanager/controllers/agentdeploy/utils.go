...
func getManifestConfigOption(agentAddon agent.AgentAddon,
	...
	for _, mc := range aaomc {
		...
		// // prune the feedback rules generated by the addon manager
		// manifestConfigs[index].FeedbackRules = pruneFeedbackRule(manifestConfigs[index].FeedbackRules)

		// merge the generated feedback rules and the user configured feedback rules
...
// // pruneFeedbackRule removes the duplicated feedback rules.
// // the returned rules only contains at most one rule for each type, in details:
// // - will deduplicate the well known status type rules.
// // - will merge the json path rules based on the name.
// func pruneFeedbackRule(rules []workapiv1.FeedbackRule) []workapiv1.FeedbackRule {
// 	var rrules []workapiv1.FeedbackRule
// 	var foundWellKnownStatus bool = false
// 	var jsonPaths []workapiv1.JsonPath

// 	for _, rule := range rules {
// 		switch rule.Type {
// 		case workapiv1.WellKnownStatusType:
// 			if foundWellKnownStatus {
// 				continue
// 			}
// 			foundWellKnownStatus = true
// 			rrules = append(rrules, rule)
// 		case workapiv1.JSONPathsType:
// 			for _, path := range rule.JsonPaths {
// 				found := false
// 				for _, rpath := range jsonPaths {
